                              ...
                              void *event_data)
{
    ...
    case GPS_UPDATE :
        ...
        gps_data.timestamp_ms = get_current_timestamp_ms();
        gps_data.hour = gps->tim.hour + TIME_ZONE;
        gps_data.minute = gps->tim.minute;
        gps_data.second = gps->tim.second;
        gps_data.day = gps->date.day;
        gps_data.month = gps->date.month;
        gps_data.year = gps->date.year + YEAR_BASE;
        gps_data.latitude = gps->latitude;
        gps_data.longitude = gps->longitude;
        gps_data.speed_mps = gps->speed;
        gps_data.altitude_m = gps->altitude;
        xQueueOverwriteFromISR(gps_queue, &gps_data, pdFALSE);
        /*   ESP_LOGI(TAG,
                    "%d/%d/%d %d:%d:%d => \r\n"
                    "\t\t\t\t\t\tlatitude   = %.05f°N\r\n"
                    "\t\t\t\t\t\tlongitude = %.05f°E\r\n"
